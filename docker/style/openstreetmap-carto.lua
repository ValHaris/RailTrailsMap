...
function filter_basic_tags_rel (keyvalues, numberofkeys)
    ...
    if ((keyvalues["type"] ~= "route") or (keyvalues["route"] ~= "bicycle")) then
        return 1, keyvalues
    end
...
function filter_tags_way (keyvalues, numberofkeys)
    ...
    if (keyvalues["abandoned:railway"] == "tram" and (excluded_tram_highways[keyvalues["highway"]])) then
